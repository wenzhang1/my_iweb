...
class IPaging
{
	...
	public function getTotalPage()
	{
		...
		return $tem="当前第 {$this->index} 页 / 共 {$this->totalpage} 页";
	}
    /**
     * @brief 设置展示的分页数量
	 * @return int 分页数量
	...
	public function getPageBar($url='', $attrs='')
	{
	      ...
	      else
	      {
        	...
        	$href = $baseUrl.($flag?1:'');
			if($this->firstpage>1)$tem="<li><a href='{$href}' {$attr}>首页</a></li>";
	      ...
	      $href = $baseUrl.($flag?$this->getIndex()-1:'');
			...
			if($this->firstpage>1){
				$tem.="<li><a href='{$href}' {$attr}>上一页</a></li>";
			}else{
				if($this->index == 1){
					$tem="<li class='disabled'>上一页</li>";
				}else{
					$tem="<li><a href='{$href}' {$attr}>上一页</a></li>";
				}
			}
			...
			for($i=$this->firstpage;$i<=$this->lastpage;$i++)
			{
				...
				if($i==$this->index)
				{
					$tem.="<li class='current'>{$i}</li>";
				}
				else
				{
					$tem.="<li><a href='{$href}' {$attr}>{$i}</a></li>";
				}
			}
